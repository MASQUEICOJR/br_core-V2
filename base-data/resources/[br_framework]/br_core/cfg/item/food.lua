...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    local name = BR.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = BR.getUserId(player)
      if user_id ~= nil then
        if BR.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then BR.varyHunger(user_id,vary_hunger) end
          if vary_thirst ~= 0 then BR.varyThirst(user_id,vary_thirst) end

          if ftype == "drink" then
            BRclient._notify(player,"~b~ Drinking "..name..".")
            play_drink(player)
          elseif ftype == "eat" then
            BRclient._notify(player,"~o~ Eating "..name..".")
            play_eat(player)
          end
