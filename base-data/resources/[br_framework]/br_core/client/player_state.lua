...
function tBR.playerStateReady(state)
	state_ready = state
end
...
--[[Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerPlaying(PlayerId()) and state_ready then
			...
			BRserver._updateHealth(tBR.getHealth())
			BRserver._updateWeapons(tBR.getWeapons())
			BRserver._updateCustomization(tBR.getCustomization())
			BRserver._updateArmor(tBR.getArmour())
		end
	end
end)]]
...
function tBR.replaceWeapons(weapons)
	local old_weapons = tBR.getWeapons()
	tBR.giveWeapons(weapons,true)
	return old_weapons
end

function tBR.giveWeapons(weapons,clear_before)
	local player = PlayerPedId()
...
function tBR.setArmour(amount)
	SetPedArmour(PlayerPedId(),amount)
end

function tBR.getArmour()
...
function tBR.getDrawables(part)
...
function tBR.getDrawableTextures(part,drawable)
	local isprop, index = parse_part(part)
	if isprop then
		...
		if custom then
			...
			if mhash then
                ...
                if HasModelLoaded(mhash) then
                    local weapons = tBR.getWeapons()
                    local armour = GetPedArmour(ped)
                    local health = tBR.getHealth()

                    SetPlayerModel(PlayerId(),mhash)
                    tBR.setHealth(health)
                    tBR.giveWeapons(weapons,true)
                    tBR.setArmour(armour)
