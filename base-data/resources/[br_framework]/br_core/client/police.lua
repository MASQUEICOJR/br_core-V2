...
function tBR.putInNetVehicleAsPassenger(net_veh)
...
function tBR.putInVehiclePositionAsPassenger(x, y, z)
  local veh = tBR.getVehicleAtPosition(x, y, z)
  if IsEntityAVehicle(veh) then
    for i = 1, GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh, i) then
        SetPedIntoVehicle(PlayerPedId(), veh, i)
...
function tBR.followPlayer(player)
...
function tBR.getFollowedPlayer()
...
function tBR.jail(x, y, z, radius)
  tBR.teleport(x, y, z) -- teleport to center
...
AddStateBagChangeHandler('handcuffed', ('player:%s'):format(cache.serverId), function(_, _, value)  
  ...
  if value then
    ...
    tBR.playAnim(true, { { "mp_arresting", "idle", 1 } }, true)
    if not handcuffThreadStarted then
      StartHandcuffThread()
    end
  else    
