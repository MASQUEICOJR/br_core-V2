...
Citizen.CreateThread(function()
	while true do
		...
		if noclip then
			...
			local dx,dy,dz = tBR.getCamDirection()
			local speed = 1.0
...
function tBR.toggleHandcuff()
	handcuffed = not handcuffed
	SetEnableHandcuffs(PlayerPedId(),handcuffed)
	if handcuffed then
		tBR.playAnim(true,{{"mp_arresting","idle"}},true)
	else
		tBR.stopAnim(true)
	end
end

function tBR.setHandcuffed(flag)
	if handcuffed ~= flag then
		tBR.toggleHandcuff()
	end
end

function tBR.isHandcuffed()
	return handcuffed
end
...
local capuz = false
function tBR.toggleCapuz()
	capuz = not capuz
	if capuz then
		tBR.setDiv("capuz",".div_capuz { background: #000; margin: 0; width: 100%; height: 100%; }","")
...
function tBR.setCapuz(flag)
	if capuz ~= flag then
		tBR.toggleCapuz()
	end
end

function tBR.isCapuz()
	return capuz
end

local mala = false
function tBR.toggleMalas()
	mala = not mala
	ped = PlayerPedId()
	vehicle = tBR.getNearestVehicle(7)
...
function tBR.setMalas(flag)
	if mala ~= flag then
		tBR.toggleMalas()
	end
end

function tBR.isMalas()
	return mala
end

function tBR.getNoCarro()
	return IsPedInAnyVehicle(PlayerPedId())
end

function tBR.getCarroClass(vehicle)
...
function tBR.putInNearestVehicleAsPassenger(radius)
	local veh = tBR.getNearestVehicle(radius)
...
function tBR.checkDistance(x2,y2,z2,h2)
	local ped = PlayerPedId()
