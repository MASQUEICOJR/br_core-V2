...
function tBR.varyHealth(variation)
  local ped = PlayerPedId()
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) then
      ...
      if IsPedOnFoot(ped) and not tBR.isNoclip() then
        local factor = math.min(tBR.getSpeed(), 10)
      ...
      -- do variation
      if vthirst ~= 0 then
        BRserver._varyThirst(vthirst / 12.0)
      end

      if vhunger ~= 0 then
        BRserver._varyHunger(vhunger / 12.0)
      end
    end
  end
end)
...
function tBR.isInComa()
  return in_coma
end

-- kill the player if in coma
function tBR.killComa()
  if in_coma then
    coma_left = 0
  end
end
