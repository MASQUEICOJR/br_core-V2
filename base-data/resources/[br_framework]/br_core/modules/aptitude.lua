...
function BR.varyExp(user_id, group, aptitude, amount)
  ...
  if def ~= nil and uaptitudes ~= nil then
    ...
    local level = math.floor(BR.expToLevel(exp)) -- save level before variation
    ...
    if player ~= nil then
      ...
      --- exp
      if amount < 0 then        
        BR.notify(player, locale('aptitude.lose_exp', group_title, aptitude_title, -1*amount), nil, 7000, 'inform')
      elseif amount > 0 then        
        BR.notify(player, locale('aptitude.earn_exp', group_title, aptitude_title, amount), nil, 7000, 'inform')
      end
      --- level up/down
      local new_level = math.floor(BR.expToLevel(exp))
      local diff = new_level-level
      if diff < 0 then
        BR.notify(player, locale('aptitude.level_down', group_title, aptitude_title, new_level), nil, 7000, 'inform')        
      elseif diff > 0 then        
        BR.notify(player, locale('aptitude.level_up', group_title, aptitude_title, new_level), nil, 7000, 'inform')
      end
      TriggerClientEvent('BR:SetPlayerData', player, BR.getPlayerInfo(user_id))
    end
  end
end

function BR.levelUp(user_id, group, aptitude)
  local exp = BR.getExp(user_id,group,aptitude)
  local next_level = math.floor(BR.expToLevel(exp))+1
  local next_exp = BR.levelToExp(next_level)
  local add_exp = next_exp-exp
  BR.varyExp(user_id, group, aptitude, add_exp)
end

function BR.levelDown(user_id, group, aptitude)
  local exp = BR.getExp(user_id,group,aptitude)
  local prev_level = math.floor(BR.expToLevel(exp))-1
  local prev_exp = BR.levelToExp(prev_level)
...
function BR.setExp(user_id, group, aptitude, amount)
  local exp = BR.getExp(user_id, group, aptitude)
  BR.varyExp(user_id, group, aptitude, amount-exp)
end
