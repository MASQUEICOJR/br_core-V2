...
--br_coreex
function BR.playSound(id, name, looped, location, maxDistance, target)    
    TriggerEvent('chHyperSound:play', id, name, looped, location, maxDistance, target)
end

function BR.stopSound(id, target)
     TriggerEvent('chHyperSound:stop', id, target)
end

function BR.playSoundOnEntity(entityNetId, soundId, soundName, isLooped, maxDistance, targetId)
