...
function BR.createNewCharacter(user_id, firstname, lastname, gender, registration, phone, bithdate, money, inventory, datatable)
    ...
    return BR.insert('BR/addPlayer',
        { user_id, firstname, lastname, gender, registration, phone, bithdate, money, inventory, datatable })
end

function BR.deleteCharacter(char_id)
    BR.update('BR/deletePlayer', { char_id })
end

function BR.updateCharacter(char_id, data)
...
function BR.getCharacter(char_id, include_deleted)
    local xPlayer
    if not include_deleted then
        xPlayer = BR.single('BR/getPlayer', { char_id })
    else
        xPlayer = BR.single('BR/getPlayerIncludeDeleted', { char_id })
    end
...
function BR.getAllUserCharacter(user_id, include_deleted)
    local xPlayers
    if not include_deleted then
        xPlayers = BR.query('BR/getAllPlayerFromUser', { user_id })
    else
        xPlayers = BR.query('BR/getAllPlayerFromUserIncludeDeleted', { user_id })
    end
...
function BR.getCharacterBy(column, value)
    local xPlayers = BR.query('BR/getPlayersBy', { column, value })
...
function BR.generateRegistrationNumber()
    ...
    repeat
        ...
        local player = BR.getCharacterBy("registration", registration)?[1]
    until not player
    return registration
end

function BR.generateRandomPhoneNumber()
    local phone = ""
    repeat
        phone = lib.string.random('111-111')
        local player = BR.getCharacterBy("phone", phone)?[1]
    until not player
    return phone
end
