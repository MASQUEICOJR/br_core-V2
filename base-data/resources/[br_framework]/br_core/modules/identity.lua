...
function BR.generateStringNumber(format) -- (ex: DDDLLL, D => digit, L => letter)
...
function BR.getPlayerIdentity(user_id, offline)
  if not next(BR.user_tables[user_id] or {}) then
    if offline then
      local character = BR.getCharacter(user_id, false)
      if character then
  ...
  local xPlayer = BR.user_tables[user_id]
  local birth_date = os.date("%d-%m-%Y", xPlayer.birth_date // 1000)
  local jobName, jobData = BR.getUserGroupByType(xPlayer.user_id, 'job')
  return {
...
function BR.getUserIdentity(user_id)
  return BR.getPlayerIdentity(user_id, false)
end
