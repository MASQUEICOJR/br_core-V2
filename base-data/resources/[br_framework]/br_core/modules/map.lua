...
-- NUMPERMISSION
-----------------------------------------------------------------------------------------------------------------------------------------
function BR.numPermission(perm)
	local users = {}
...
function BR.setArea(source,name,x,y,z,radius,height,cb_enter,cb_leave)
	local areas = client_areas[source] or {}
	client_areas[source] = areas
	areas[name] = { enter = cb_enter, leave = cb_leave }
	BRclient._setArea(source,name,x,y,z,radius,height)
end

function BR.removeArea(source,name)
	BRclient._removeArea(source,name)
	local areas = client_areas[source]
	if areas then
...
function tBR.enterArea(name)
	local source = source
	local user_id = BR.getUserId(source)
	local areas = client_areas[source]
	if not BR.searchReturn(source,user_id) then
		if areas then
			local area = areas[name] 
...
AddEventHandler("BR:playerSpawn",function(user_id,source,first_spawn)
	if first_spawn then
		for k,v in pairs(cfg.blips) do
			BRclient._addBlip(source,v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8])
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(search) do
			if v > 0 and BR.getUserSource(k) then
				search[k] = v - 1
				if v == 0 then
					search[k] = nil
				end
...
-- SEARCHRETURN
-----------------------------------------------------------------------------------------------------------------------------------------
function BR.searchReturn(source,user_id)
	if search[user_id] == 0 or not search[user_id] then
		return false
	else
		TriggerClientEvent("Notify",source,"aviso","Aguarde "..BRclient.getTimeFunction(source,parseInt(search[user_id]))..".",8000)
		return true
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SEARCHTIMER
-----------------------------------------------------------------------------------------------------------------------------------------
function BR.searchTimer(user_id,timer)
	if search[user_id] == 0 or not search[user_id] then
...
RegisterCommand('procurado',function(source,args,rawCommand)
	...
	if user_id then
		if search[user_id] == 0 or not search[user_id] then
			TriggerClientEvent("Notify",source,"aviso","Você não está sendo procurado.",10000)
		else
			TriggerClientEvent("Notify",source,"aviso","Aguarde "..BRclient.getTimeFunction(source,parseInt(search[user_id]))..".",8000)
		end
	end
