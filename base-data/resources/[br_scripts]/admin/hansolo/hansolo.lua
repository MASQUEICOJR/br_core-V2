...
SVIDclient = Tunnel.getInterface("br_core_admin")

src = {}
Tunnel.bindInterface("adminastro",src)
...
AddEventHandler('admcuff',function()
	...
	if BR.isHandcuffed() then
		BR._setHandcuffed(source,false)
		SetPedComponentVariation(PlayerPedId(),7,0,0,2)
	end
end)

RegisterNetEvent('deattach')
AddEventHandler('deattach',function()
	local ped = PlayerPedId()
	TriggerServerEvent("br_core_policia:carregarsolo")
end)
...
function src.vehClass()
	local veh = GetVehiclePedIsIn(PlayerPedId(), false)
	return GetVehicleClass(veh)
end

function src.vehHash()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	return GetEntityModel(vehicle),GetEntityModel(vehicle)
end
...
AddEventHandler("vehash",function()
	local vehicle = BR.getNearestVehicle(7)
    if IsEntityAVehicle(vehicle) then
        BR.prompt(source,"HASH: "..GetEntityModel(vehicle),GetEntityModel(vehicle))
	end
end)

--[[RegisterNetEvent('spawnvehcoords')
AddEventHandler('spawnvehcoords',function(name,x,y,z,h)
	print('sim')
	...
	if HasModelLoaded(mhash) then
		...
		SetVehicleNumberPlateText(nveh,"FIVE-Z")
...
--[[local garagens = {
    { -158.81,-2495.22,49.24},
    { -158.62,-2508.37,48.48},
    { -146.52,-2505.99,47.67},
    { -144.16,-2519.28,46.66},
}
...
function spawnVehicle(mhash,x,y,z,h)
    ...
    SetVehicleOnGroundProperly(nveh)
		...
		SetVehRadioStation(nveh,"OFF")
		SetVehicleDirtLevel(nveh,0.0)
		SetVehicleDoorsLocked(nveh,locked)
		SetVehicleEngineHealth(nveh,0)
    return nveh
end
...
Citizen.CreateThread(function()
    while true do
		local x,y,z,h
		local rand = math.floor(math.random() * #garagens);
		if rand == 0 then rand = 1 end
		x = garagens[rand][1]
		y = garagens[rand][2]
		z = garagens[rand][3]
		h = 156.87
		local rand2 = math.floor(math.random() * #carros);
		if rand2 == 0 then rand2 = 1 end
		spawnVehicle(carros[rand2][1],x,y,z,h)
		Citizen.Wait(20000)
    end
end)]]
    ...
    while true do
        ...
        if dickheaddebug then
            ...
            if IsControlJustReleased(0, 38) then
                if inFreeze then
                    inFreeze = false
                    BR.notify("Freeze Disabled")
                else
                    inFreeze = true
                    BR.notify("Freeze Enabled")
                end
            end
        else
            Citizen.Wait(5000)
...
AddEventHandler('setmochila',function(modelo,cor)
	...
	if GetEntityHealth(ped) > 101 then
		if not modelo then
			BR._playAnim(true,{{"clothingshirt","try_shirt_positive_d"}},false)
			Wait(2500)
			ClearPedTasks(ped)
