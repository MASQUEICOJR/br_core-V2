    ...
    for ply_id in pairs(Organizations.MembersList[user.groupType]) do
        ...
        if Organizations.Members[ply_id] then
            if (not Organizations.Members[ply_id].joindate or not Organizations.Members[ply_id].lastlogin) then
                ...
                local query = BR.query('br_orgs/GetAllUserInfo', { user_id = ply_id })
                if #query > 0 then
                    ...
                    exports["oxmysql"]:execute([[INSERT IGNORE INTO orgs_player_infos(user_id, organization, joindate, lastlogin) VALUES(?,?,?,?)]], { ply_id, user.groupType, timestamp, timestamp })
                end
            end
        end
        local groupsGet = {}
...
function RegisterTunnel.inviteMember(playerId)
    ...
    if request then
        ...
        SetTimeout(500, function()
            if Organizations.Members[ply_id] then
                BR.execute('br_orgs/DeleteUserInfo', { user_id = ply_id })
                ...
                exports["oxmysql"]:execute([[INSERT IGNORE INTO orgs_player_infos(user_id, organization, joindate, lastlogin, timeplayed) VALUES(?,?,?,?,?)]], { ply_id, user.groupType, Organizations.Members[ply_id].joindate, Organizations.Members[ply_id].lastlogin, Organizations.Members[ply_id].timeplayed })
            end
        end)
...
function BLACKLIST:checkUser(user_id)
    local blacklist = BR.getUData(user_id, 'facs:blacklist') or 0
    return tonumber(blacklist) or 0
end

function BLACKLIST:addUser(user_id)
    BR.setUData(user_id,'facs:blacklist', os.time() + Config.Main.blackList * 86400)
end

function BLACKLIST:remUser(user_id)
    BR.setUData(user_id,'facs:blacklist', 0)
end
