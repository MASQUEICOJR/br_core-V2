...
local BRConfig = require('@br_core.cfg.base')

local players = {}
local firstcreation = {}

local function GiveInitialItems(source)
    if BRConfig?.initial_items then
        local user_id = BR.getUserId( source )
        for item, amount in next, BRConfig?.initial_items or {} do            
            if item == 'phone' then
                local phoneNumber = BR.getPlayerTable(user_id)?.phone
                if phoneNumber then
                    pcall(
                        function()
...
lib.callback.register('multichar:server:requestCharsInfo', function(source)
    ...
    local characters = BR.getAllUserCharacter(user_id, true)
    local _characters = {}
    ...
    -- BR.setPlayerBucket(source, source + 1, false)
    local max_chars = tonumber(BR.getUData(user_id, 'br_core:max_char') or 1) or 1    
...
lib.callback.register("multichar:server:createchar", function(source, firstname, lastname, date, gender)
    ...
    local logged = BR.login(source, user_id, char_id, true)

    repeat
        Wait(1000)
    until firstcreation[source]

    -- BR.setPlayerBucket(source, 0, false)
...
lib.callback.register("multichar:server:login", function(source, char_id)
    if BR.login(source, BR.getUserId(source), char_id, false) then
        local xPlayer = BR.getPlayerInfo(source)
...
AddEventHandler('br_core:login', function(source, user_id, char_id, created)
    Wait(1000)
