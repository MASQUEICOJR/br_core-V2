...
if multiCharacter then
    BR._prepare("nation_creator/createAgeColumn","ALTER TABLE user_identities ADD IF NOT EXISTS age INT(11) NOT NULL DEFAULT 20")
    BR._prepare("nation_creator/update_user_first_spawn","UPDATE user_identities SET firstname = @firstname, name = @name, age = @age WHERE user_id = @user_id")
    BR._prepare("nation_creator/create_characters","INSERT INTO users(steam) VALUES(@steam)")
    BR._prepare("nation_creator/remove_characters","UPDATE users SET deleted = 1 WHERE id = @id")
    BR._prepare("nation_creator/get_characters","SELECT * FROM users WHERE steam = @steam and deleted = 0")
    BR._prepare("nation_creator/get_character","SELECT * FROM users WHERE steam = @steam and deleted = 0 and id = @user_id")
    BR._prepare("nation_creator/get_charinfo","SELECT * FROM user_identities WHERE user_id = @user_id")
    CreateThread(function() BR.execute("nation_creator/createAgeColumn") end) -- criar coluna idade na db
else
    BR._prepare("nation_creator/update_user_first_spawn","UPDATE user_identities SET firstname = @firstname, name = @name, age = @age WHERE user_id = @user_id")
end
...
function func.checkPermission(permission, src)
    ...
    if type(permission) == "table" then
        for i, perm in pairs(permission) do
            if BR.hasPermission(user_id, perm) then
                return true
            end
        end
        return false
    end
    return BR.hasPermission(user_id, permission)
end


function func.saveChar(name, lastName, age, char, id)
    local source = source
    local user_id = id or BR.getUserId(source)
    if char then
        BR.setUData(user_id, "nation_char", json.encode(char,{indent=false}))
    end
    if name and lastName and age then
        BR.execute("nation_creator/update_user_first_spawn",{ user_id = user_id, firstname = lastName, name = name, age = age })
    end
    TriggerClientEvent("nation_barbershop:init", source, char)
    BR._updateSelectSkin(user_id, GetEntityModel(GetPlayerPed(source)))
    return true
end
...
AddEventHandler("BR:playerSpawn",playerSpawn)
...
function processSpawnController(source,char,user_id)
	...
	if char then
        ...
        userlogin[user_id] = true
        local data = BR.getUData(user_id, "currentCharacterMode")
        if data and data ~= "" then 
            local gender = "male"
            local char = json.decode(data)
            if char and char.gender and char.gender == 1 then
...
function getUserLastPosition(source, user_id)
    ...
    if datatable and datatable.position then
        ...
        local data = BR.getUData(user_id, "Datatable")
        if data and data ~= "" then
...
function func.getCharsInfo()
    ...
    local data = BR.query("nation_creator/get_characters",{ steam = steam })
    local info = { chars = {} }
    for k,v in ipairs(data) do
        local teste = BR.query("nation_creator/get_charinfo",{ user_id = v.id })
        local bank = BR.getBankMoney(v.id)
...
function getUserClothes(user_id)
    local data = BR.getUData(user_id, "Clothings")
    if data and data ~= "" then
    ...
    data = BR.getUData(user_id, "BR:datatable")
    if data and data ~= "" then
    ...
    local datatable = BR.getUserDataTable(user_id) or {}
    return datatable.customization or {}
end

function getUserTattoos(user_id)
    local data = BR.getUData(user_id,"BR:tattoos")
    if data and data ~= '' then
       local custom = json.decode(data)  
       return custom or {}
    end
    data = BR.getUData(user_id,"Tattoos")
    if data and data ~= '' then
       local custom = json.decode(data)  
       return custom or {}
    end
...
function getGender(user_id)
    local datatable = BR.getUserDataTable(user_id) or json.decode(BR.getUData(user_id, "Datatable")) or {}
...
function func.playChar(info)
    local source = source
    local steam = getPlayerSteam(source)
    local data = BR.query("nation_creator/get_character",{ steam = steam, user_id = info.user_id })
    if #data > 0 then
...
function func.tryDeleteChar(info)
    ...
    local data = BR.query("nation_creator/get_character",{ steam = steam, user_id = info.user_id })
    if #data > 0 then
        BR._execute("nation_creator/remove_characters",{ id = info.user_id })
        return true, ""
    end
    return false, "error"
end
...
function func.tryCreateChar()
    ...
    if #data < getUserMaxChars(source)  then -- limite de personagens
        BR.execute("nation_creator/create_characters",{ steam = steam })
        local myChars = BR.query("nation_creator/get_characters",{ steam = steam })
...
RegisterCommand('resetchar',function(source, args) -- COMANDO DE ADMIN PARA RESETAR PERSONAGEM
    if func.checkPermission({"admin.permissao", "manager.permissao", "Admin"}, source) then
        if args[1] then 
            ...
            if id then
                local src = BR.getUserSource(id)
                if src and BR.request(source, "Deseja resetar o id "..id.." ?", 30) then
                    fclient._startCreator(src)
                end
            end
        elseif BR.request(source, "Deseja resetar seu personagem ?", 30) then
            fclient._startCreator(source)
        end
    end
end)
...
RegisterCommand('spawn',function(source) -- COMANDO DE ADMIN PARA SIMULAR O SPAWN
    if func.checkPermission({"admin.permissao", "mod.permissao", "Admin"}, source) or not BR.getUserId(source) then
        if multiCharacter then
            ...
            playerSpawn(BR.getUserId(source), source, true)
        end
    end
end)
